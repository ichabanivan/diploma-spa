...
import { get, isString, isArray, find, isRegExp, isFunction } from 'lodash';
...
export default class Route {
    ...
    static getSearch = () => String(get(history, 'location.search') || '');

    static getPathname = () => String(get(history, 'location.pathname') || '');
    ...
    static defineREGEXP = (url, custom) => (isRegExp(custom) ? custom
      : new RegExp(String(url).replace(Route.regParam, '.*'), 'i'));

    static defineIsActive = (regexp, custom) => (isFunction(custom) ? custom
      : () => regexp.test(Route.getPathname()));

    static defineFormatPath = (url, annotation, custom) => (isFunction(custom) ? custom : options => {
      const params = {};
    ...
    static defineParsePath = (url, annotation, custom) => (isFunction(custom) ? custom : path => {
      const result = {};
      const matcher = new RegExp(String(url).replace(Route.regParam, (a, propName) => `(?<${propName}>[^/]+)`), 'i');
      const params = get(String(path).match(matcher), 'groups', {});
      annotation.map(param => param.from(params, result));
      return result;
    });

    static defineFormatQuery = (annotation, custom) => (isFunction(custom) ? custom : params => {
      const result = {};
      annotation.map(qp => qp.to(params, result));
      return qs.stringify(result, { addQueryPrefix: true });
    });

    static defineParseQuery = (annotation, custom) => (isFunction(custom) ? custom : queryString => {
      const result = {};
    ...
    static defineQueryAnnotation = (url, options) => {
      ...
      if (isArray(options)) {
        options.map(qp => {
          ...
          const exist = find(annotation, { name: qp.name });
    ...
    static defineParamsAnnotation = (url, options) => {
      ...
      if (isArray(options)) {
        options.map(rp => {
          ...
          const exist = find(annotation, { name: rp.name });
    ...
    PARAMS = pathname => this[Route.secret].parsePath(isString(pathname) ? pathname : Route.getPathname());

    QUERY = search => this[Route.secret].parseQuery(isString(search) ? search : Route.getSearch());
